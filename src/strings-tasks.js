...
function repeatString(str, times) {
  if (times < 0) return str.repeat(0);
...
function removeFirstOccurrences(str, value) {
  if (str.indexOf(value) !== -1) {
    ...
    const b = str.substring(str.indexOf(value) + value.length, str.length);
    return a.concat(b);
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  if (str.lastIndexOf(value) !== -1) {
    const a = str.substring(0, str.lastIndexOf(value));
    const b = str.substring(str.lastIndexOf(value) + value.length, str.length);
    return a.concat(b);
  }
  return str;
}
...
function isPalindrome(str) {
  ...
  const str6 = str5.replace(/[^a-zA-Z]+/g, '');
  if (str.toLowerCase().replace(/[^a-zA-Z]+/g, '') === str6) {
    return true;
  }
  return false;
}
...
function findLongestWord(sentence) {
  ...
  const a = sen.map((e) => e.length);
  const b = sen.find((e) => e.length >= Math.max(...a));
  return b;
}
...
function reverseWords(str) {
  const s = str.split(' ');
  const a = s.map((e) => e.split('').reverse().join(''));
  return a.join(' ');
}
